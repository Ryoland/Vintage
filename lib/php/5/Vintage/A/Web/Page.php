      ...
      final protected function queries2() {
        ...
        if (!empty(static::$QUERIES)) {
          ...
          foreach (static::$QUERIES as $name => $QUERY) {
            ...
            $value  = null;
            $method = isset($QUERY['method']) ?  $QUERY['method'] : static::$QUERY_METHOD;
          ...
          list($queries, $errors) = Check::run($queries, static::$QUERIES);
        }
          ...
          foreach (static::$QUERIES2 as $key => $conf) {
            ...
            if (!isset($errors[$key])) {
              ...
              if ($type1 == 'array') {
                ...
                if (strlen($type2)) {
                  switch ($type1) {
                    case 'integer' : $value = VInteger::cast($type2, $value);
                    break;
                  }
                }
              }
...
// ##
      ...
      final protected function display_errors(array $errors = array(), $title = '') {
      ...
      final protected function fetch_errors(array $errors = array(), $title = '') {
